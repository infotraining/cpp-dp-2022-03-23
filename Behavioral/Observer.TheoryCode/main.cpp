...
// Observer & Subject
class Fan : public Observer<TemperatureMonitor, double>, public Observable<Fan, std::string>
{
    bool is_on_ = false;

public:
    void update(TemperatureMonitor& tm, double current_tempreture) override
    {
        if (!is_on_ && current_tempreture > 25.0)
            on();

        if (is_on_ && current_tempreture < 24.0)
            off();
    }
...
// Observer
class ConsoleLogger : public Observer<TemperatureMonitor, double>, public Observer<Fan, std::string>
{
public:
    void update(TemperatureMonitor& tm, double current_tempreture) override
    {
        std::cout << "ConsoleLogger notified - temperature has changed to " << current_tempreture << std::endl;
    }

    void update(Fan& tm, std::string message) override
    {
